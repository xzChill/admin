...
namespace Chill.DataRepository
{
    internal class ShardingQueryable<T> : IShardingQueryable<T> where T : class, new()
    {
        ...
        {
            var list = GetStatisData(x => new KeyValuePair<int, dynamic>(x.Count(), Chill.Common.Extention.DynamicSum(x, selector))).Select(x => (KeyValuePair<int, dynamic>)x).ToList();
        ...
        {
            return GetStatisData(x => Chill.Common.Extention.DynamicSum(x, selector)).Sum(x => (decimal?)x);
        }
